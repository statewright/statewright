...
The operator handles lifecycle concerns, not event processing:
...
This boundary is load-bearing. The operator touches etcd at infrastructure timescales (seconds). The data plane operates at application timescales (milliseconds). Mixing them hits etcd's hard limits (~30-40k objects, 1MiB per object, 100-500ms reconcile overhead).
